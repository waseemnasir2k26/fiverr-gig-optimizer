...
5. Open `fiverr-catalog.html` in your browser
...
├── build-catalog.py              # Main build script
...
2. **`build-catalog.py`** reads the config and generates a single HTML file with:
   ...
   - Copy-paste buttons for every field
   - PDF/text export per gig
   - Print-friendly layout

3. **You** open the HTML, review everything, download thumbnails, and create gigs on Fiverr by copying the optimized content.
